        ...
        <div className="max-w-3xl mx-auto text-center space-y-12">
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-gold/30 bg-gold/5">
              ...
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-gold">{t('newsletter.badge')}</span>
            </div>
            
            <h2 className="text-5xl md:text-7xl font-serif tracking-tighter leading-tight text-balance">
              {t('newsletter.title')}<span className="italic text-gold">{t('newsletter.titleItalic')}</span>
            </h2>
          ...
          {!subscribed ? (
            ...
            >
              ...
              <h3 className="text-2xl font-serif text-gold">{t('newsletter.successTitle')}</h3>
              <p className="text-gray-400 font-light italic">{t('newsletter.successDesc')}</p>
            </motion.div>
          )}
          ...
          <div className="pt-12 flex flex-wrap justify-center gap-10 opacity-40 grayscale group-hover:grayscale-0 transition-all duration-1000">
            <span className="text-[10px] uppercase tracking-widest font-bold">{t('newsletter.label1')}</span>
            <span className="text-[10px] uppercase tracking-widest font-bold">{t('newsletter.label2')}</span>
            <span className="text-[10px] uppercase tracking-widest font-bold">{t('newsletter.label3')}</span>
          </div>
        </div>
      </div>
    </section>
